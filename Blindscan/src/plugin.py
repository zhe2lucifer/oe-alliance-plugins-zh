	...
	def createConfig(self):
		...
		for n in nimmanager.nim_slots:
			...
			if len(nimmanager.getSatListForNim(n.slot)) < 1: # empty setup
				# Do Nothing, to fix a crash issue
...
#		print "[Blindscan][doClock] self.is_runable", self.is_runable
	...
	def prepareScanData(self, orb, pol, band, is_scan):
		...
		if tunername in _blindscans2Nims:
			...
			exe_filename = "ali_blindscan"
			exe_path = "/usr/bin/%s" % exe_filename
			if os.path.exists(exe_path):
				cmd = "%s --start=%d --stop=%d --min=%d --max=%d --slot=%d --i2c=%d" % (exe_filename, temp_start_int_freq, temp_end_int_freq, config.blindscan.start_symbol.value, config.blindscan.stop_symbol.value, self.feid, self.getNimSocket(self.feid))
				if self.user_defined_lnb_scan:
					cmd += " --lof=%d" % self.user_defined_lnb_lo_freq
					self.adjust_freq = False
				if tab_pol[pol]:
					cmd += " --vertical"
				if self.is_c_band_scan:
	...
	def dataIsGood(self, data): # check output of the binary for nonsense values
		...
		#print "self.c_band_lo_freq:%d, c_high:%d, c_low:%d"%(self.c_band_lo_freq, self.c_band_freq_limits["high"], self.c_band_freq_limits["low"])
		#print "self.adjust_freq:%d,self.user_defined_lnb_lo_freq:%d, low_freq:%d"%(self.adjust_freq, self.user_defined_lnb_lo_freq, self.universal_lo_freq["low"])
		#print "high_freq:%d, is_c_band_scan:%d, high_band:%d, self.user_defined_lnb_scan:%d"%(self.universal_lo_freq["high"],self.is_c_band_scan,high_band ,self.user_defined_lnb_scan)
		#print "Data: Frequency [%d], Symbol rate [%d] lower_freq:%d data_if_req:%d lower_symbol:%d, data_symbol:%d" % (int(data[2]), int(data[3]),lower_freq, data_if_freq, lower_symbol, data_symbol)
